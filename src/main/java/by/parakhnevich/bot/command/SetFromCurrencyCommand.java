...
public class SetFromCurrencyCommand implements Command{
    MessageService service;
    ConvertCommand command;
    CurrencyValidator validator;

    public SetFromCurrencyCommand(ConvertCommand command) {
    ...
    public String execute(Message message) throws BadCurrencyException {
        ...
        command.getCurrenciesFrom().put(message.getChatId(),
                Currency.valueOf(param.toUpperCase(Locale.ROOT)));
        return "Setted " + param.toUpperCase(Locale.ROOT) + " as from currency";
    }
}
